...
def main():
    ...
    if category == "游戏":
        ...
        if note_folder_found and game_folder_found:
            ...
            create_game_folder = (
                input("是否创建游戏文件夹？(回车创建，其他键跳过): ").strip().lower()
            )
            ...
            os.startfile(target_folder)
            create_note_file = (
                input("是否创建笔记文件/Markdown？(回车创建，其他键跳过): ")
                .strip()
                .lower()
            )
            if create_note_file == "":
            ...
            create_folder_md = (
                input(
                    "是否创建文件夹和Markdown文件？(回车一起创建，1单独创建文件夹，2单独创建Markdown): "
                )
                .strip()
                .lower()
            )
    ...
    if existing_file:
        ...
        create_md = (
            input("是否创建 Markdown 文件？(回车或1创建, 其它键取消): ").strip().lower()
        )
